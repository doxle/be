...
# Create or update the Step Functions state machine used by async count reconciliation.
#
# Required:
#   WORKER_FUNCTION_ARN  - Lambda ARN used by Step Functions (function or alias ARN)
#
# Optional:
#   STATE_MACHINE_NAME   - defaults to doxle-block-reconcile-counts
...
STATE_MACHINE_NAME=${STATE_MACHINE_NAME:-doxle-block-reconcile-counts}
ROLE_ARN=${ROLE_ARN:-}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
ASL_TEMPLATE="${BE_ROOT}/stepfunctions/block_reconcile_counts_state_machine.asl.json"
...
echo "RECONCILE_STATE_MACHINE_ARN=${STATE_MACHINE_ARN}"
