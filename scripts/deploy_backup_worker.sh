...
# Build and deploy the backup worker Lambda (provided.al2023, arm64).
# Usage: ./scripts/deploy_backup_worker.sh [zip_path] [alias]

FN=${FN:-doxle-backup-worker}
ALIAS=${ALIAS:-prod}
DESKTOP_ZIP=${DESKTOP_ZIP:-"$HOME/Desktop/bootstrap-backup-worker.zip"}
...
echo "[pre] Cleaning previous backup worker build artifacts..."
rm -f "${ZIP}" "${DESKTOP_ZIP}"
cd "${BE_ROOT}"
cargo clean -p doxle-backup-worker || true

echo "📦 Building backup worker Lambda..."
echo "Running: cargo lambda build -p doxle-backup-worker --release --arm64 --output-format zip"
cargo lambda build -p doxle-backup-worker --release --arm64 --output-format zip
