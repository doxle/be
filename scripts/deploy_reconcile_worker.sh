...
# Build and deploy the reconcile worker Lambda (provided.al2023, arm64).
# Usage: ./scripts/deploy_reconcile_worker.sh [zip_path] [alias]

FN=${FN:-doxle-reconcile-worker}
ALIAS=${ALIAS:-prod}
DESKTOP_ZIP=${DESKTOP_ZIP:-"$HOME/Desktop/bootstrap-reconcile-worker.zip"}
...
echo "[pre] Cleaning previous reconcile worker build artifacts..."
rm -f "${ZIP}" "${DESKTOP_ZIP}"
cd "${BE_ROOT}"
cargo clean -p doxle-reconcile-worker || true

echo "📦 Building reconcile worker Lambda..."
echo "Running: cargo lambda build -p doxle-reconcile-worker --release --arm64 --output-format zip"
cargo lambda build -p doxle-reconcile-worker --release --arm64 --output-format zip
